...
# 権限 chmod +x github_milestone_generator_linux.sh
# 実行 ./github_milestone_generator_linux.sh
...
delete_all_milestones() {
    local username="$1"
    local repo="$2"
    local milestones=$(github_api GET "/repos/$username/$repo/milestones?state=all" | jq -r '.[].number')

    if [ -z "$milestones" ]; then
        echo "削除可能なマイルストーンがありません。"
        return
    fi

    echo "以下のマイルストーンをすべて削除します："
    github_api GET "/repos/$username/$repo/milestones?state=all" | jq -r '.[] | "\(.number): \(.title)"'
...
if ! command -v jq &>/dev/null; then
    ...
    echo "インストール方法: sudo apt-get install jq"
    exit 1
fi
